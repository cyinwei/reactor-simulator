...
void Rxbuild::get_tuples_for_slice(vector<vtkSmartPointer<vtkFloatArray>>& Farray){
	int b=0;
	float h[1];
	double L=13;//flow rate factor
	string f="FlowRate";
	...
	for(int z=0;z<14;++z){int point=0;
		if(z<13)b=z;
			...
			for(int i=0;i<89;++i){
				t->InsertTuple3(point+z*224,tran[j][i+b*193],0,0);
				if(point==3||point==13||point==26||point==40||point==55||point==71||point==87){
					++point;
					t->InsertTuple3(point+z*224,tran[j][i+b*193],0,0);
				//t->InsertTuple3(point+z*224,1,1,1);
				}
				++point;
		}		
		for(int i=89;i<104;++i){//problem is here
			...
			if(point==103){
				t->InsertTuple3(point+1+z*224,tran[j][i+b*193],0,0);
				t->InsertTuple3(point+17+z*224,tran[j][i+b*193],0,0);
				++point;
			}
			++point;
		}
